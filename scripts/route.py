...
def resolve(
    ...
    *,
    ...
    if max_age_days is not None:
        ...
        for kind, status in (("model", model_status), ("agent", agent_status)):
            if kind == "agent" and role == "none":
                continue
            match = re.search(r"\b(\d{4}-\d{2}-\d{2})\b", status)
            if not match:
                reasons.append(f"stale {kind} mapping: verification date missing")
                continue
            verified_on = datetime.strptime(match.group(1), "%Y-%m-%d").date()
            if (current - verified_on).days > max_age_days:
                reasons.append(f"stale {kind} mapping: verified {verified_on.isoformat()}")
    ...
    return {
        ...
        "harness": harness,
        "class": model_class,
        "model": model if model_verified and not any("model mapping" in reason for reason in reasons) else None,
        "role": role,
        "agent": agent if agent_verified and not any("agent mapping" in reason for reason in reasons) else None,
...
def main() -> int:
    ...
    decision = resolve(
        ...
        available_agents=set(args.available_agents) if args.available_agents is not None else None,
